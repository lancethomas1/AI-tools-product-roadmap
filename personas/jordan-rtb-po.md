...
| **Handoff runbook generator** (Later) | Primary (as receiver) | Determines what he inherits. Produced by the feature squad at release; Jordan is its quality bar and signs off before accepting ownership. A hard gate keeps him from inheriting a hollow runbook, and its outputs seed his other three tools (SLA policy, common issues, failure-mode priors). ([spec](../specs/handoff-runbook-generator.md)) |
...
| 0.3 | 2026-05-30 | Lance | Added the handoff runbook generator to the tool-fit table as Jordan's inbound/receiver tool — the build→run handoff gate that determines what RTB inherits and seeds his other three tools. |
